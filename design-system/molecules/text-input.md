...
Ce type de saisie est utilisé lorsque l'information concernée ne peut pas être standardisée et donc choisie par l'utilisateur dans une liste pré-établie.
...
Nous avons sélectionné trois types de saisie textuelle :

- la saisie sur une ligne
- la saisie sur plusieurs lignes
- la saisie dans un éditeur texte-riche (WYSYWYG)

Les contrôles qui peuvent s'appliquer peuvent être de nature très diverse, mais on peut en citer quelques-uns :

- longueur maximale
- longueur minimale
- format imposé (majuscule, minuscule, expression régulière...)
...
- Lorsque vous souhaitez donner accès à une information textuelle en lecture, privilégiez l'utilisation du composant en mode "consultation" plutôt que le composant en mode "édition" désactivé.
- Quand il existe une option nativement supportée par HTML5 pour votre besoin, privilégiez-la toujours par rapport à du code spécifique ou une fonction fournie par la bibliothèque de composants afin d'améliorer la sécurité et la compatibilité de votre application.
