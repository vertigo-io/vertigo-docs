...
Il est également possible de proposer des actions sur chaque ligne de manière contextuelle à l'élément.

Nous avons fait le choix de faire en sorte qu'un tableau puisse avoir un rendu similaire au bloc pour qu'ils puissent être placés à proximité (en général au-dessus ou en dessous) d'un bloc ou d'un autre tableau.
Ils peuvent donc avoir un titre optionnel ainsi que des actions en haut à droite.

Dans certains cas il peut être utile de pouvoir sélectionner certains lignes, dans ce cas une case à cocher de selection est présente dans une première colonne dédiée.
...
- Limiter le nombre d'actions disponibles en haut à droite (<3>), pour les actions complexes préférez une navigation vers un écran dédié
