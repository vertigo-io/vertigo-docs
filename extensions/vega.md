...
Pour le mode serveur intégré (optionnel), ajouter la feature `embeddedServer` du connecteur Javalin :
...
?> Vega peut être lancé en mode serveur intégré via la feature `embeddedServer` de `JavalinFeatures`, dans ce cas il est inutile de spécifier un filtre dans le *web.xml*
