...
Il existe des *Plugins* d'analytics qu'il est possible de brancher à la collecte des données. Ces *Plugins* ont pour but de réceptionner les données collectées et de les traiter. Il est possible de positionner autant de *Plugins* que voulu afin de s'adapter aux besoins du projet.

Les connecteurs inclus dans vertigo sont :

- `SmartLoggerAnalyticsConnectorPlugin` : qui permet de logger de manière intelligente les informations, notamment à destination du développeur. Ce connecteur permet très simplement de connaitre le nombre d'appels à la base de données pour chaque appel de webservice ou d'écran.
...
- ~~Chronograf de la suite InfluxData TICK (déprécié) : https://www.influxdata.com/time-series-platform/chronograf/~~
...
Il existe des écrans dédiés aux modules suivants : **vertigo-commons**, **vertigo-datastore**, **vertigo-vega**, **vertigo-ui** qui présente les éléments saillants relatifs à ces modules.
