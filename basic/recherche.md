...
- `indexCopyTo` : Ajoute un champ agrégeant une liste de champ du document, utilise la fonction de copy optimisée du moteur (le champ doit préexister dans le DtIndex)
...
Pour lancer une recherche Vertigo a générer du code dans le DAO du KeyConcept associé à l'index. Il faut d'abord créer une SearchQuery et la faire exécuter par le DAO.
...
Le DslListFilterBuilder vise à construire la requête Lucene de la manière la plus intuitive pour le développeur :
...
Dans tous les cas une syntaxe incomplète saisie par l'utilisateur sera échappée : les ( ), { } ou [ ] mal fermées, ou les AND OR mal utilisés.
...
* Les mots clés fournit par l'utilisateur ne sont pas forcément dans le même champ.
...
`+(field1:#+query*# field2:#+query*#)` <br/>
**KO** : les mots sont en `Obligatoire` et tous les mots saisis doivent être présent dans le même champ.
